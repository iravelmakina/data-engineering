...
| 5 | Incremental predicate | Both `fct_daily_revenue` and `fct_restaurant_daily` use `incremental_predicates=["DBT_INCREMENTAL_TARGET.revenue_date >= current_date - interval '7 days'"]`. `unique_key` must be a **list** for the `delete+insert` template to alias the target as `DBT_INCREMENTAL_TARGET` (a single-string `unique_key` produces an aliasless `DELETE … WHERE col IN (…)`) |
| 6 | Custom macro (+2.5) | `macros/standardize_text.sql` — `nullif(trim(lower(col)), '')`, used across all 6 staging models |
| 7 | Window functions in ≥2 models | Used in **5 models**: `fct_daily_revenue`, `rpt_restaurant_daily_rank`, `rpt_customer_30_day_average_spend`, `rpt_menu_item_sales_momentum`, `rpt_daily_new_customers` |
| 8 | Tests | Generic (`unique`, `not_null`, `relationships`, `accepted_values`) in YAML + 4 singular SQL tests in `tests/marts/` — **all checks pass (PASS=139)** |
| 9 | Style guide | Follows the dbt style guide: pluralized model names, `<object>_id` primary keys cast to `string`, `is_*` booleans, `<event>_at` timestamps, `<event>_date` dates, `_usd` / `_km` / `_min` unit suffixes, business terminology (`restaurant_name`, `ordered_at`), ids → strings → numerics → booleans → dates → timestamps column order, CTE names referenced without single-letter aliases, `_staging__models.yml` / `_marts__models.yml` / `_reports__models.yml` documentation |
...
```
Expected outcome on a clean run: **PASS=139, ERROR=0**.
...
│       ├── fct_daily_revenue.sql             # incremental + window fn + predicate
│       ├── fct_restaurant_daily.sql          # incremental + predicate
│       ├── reports/
│       │   ├── rpt_restaurant_daily_rank.sql
│       │   ├── rpt_courier_average_delivery_time.sql
│       │   ├── rpt_customer_30_day_average_spend.sql
...
| `fct_daily_revenue` | one row per `revenue_date` | `['revenue_date']` (list form, see note below) | recompute last 7 days **+ incremental_predicate** |
| `fct_restaurant_daily` | one row per `(restaurant_id, revenue_date)` | `['restaurant_id', 'revenue_date']` | recompute last 7 days **+ incremental_predicate** |

### Incremental predicate — `fct_daily_revenue` & `fct_restaurant_daily`

Both daily-aggregate facts use the same pattern. Example from `fct_restaurant_daily`:
...
`fct_daily_revenue` uses the same predicate with `unique_key=['revenue_date']`.
...
**dbt-duckdb gotcha:** the `delete+insert` template only aliases the target table as `DBT_INCREMENTAL_TARGET` when `unique_key` is a **list** (the multi-key path renders `DELETE … USING … AS DBT_INCREMENTAL_TARGET`). With a scalar `unique_key='revenue_date'`, the template renders `DELETE FROM target WHERE col IN (…)` with no alias and the predicate fails with `Referenced table "DBT_INCREMENTAL_TARGET" not found`. That is why both models use the list form.
...
| `rpt_customer_30_day_average_spend` | `avg() over (... rows between 29 preceding and current row)` | Rolling 30-day average per customer |
...
```sql
select courier_id, average_delivery_duration_min, difference_from_overall_average_min
from main_marts.rpt_courier_average_delivery_time
order by difference_from_overall_average_min;
```
Negative `difference_from_overall_average_min` = faster than average. Drives **bonus eligibility** and **retraining lists**.

### 4. Customer rolling spend (30-day momentum)
```sql
select customer_id, order_date, rolling_30_day_average_spend_usd
from main_marts.rpt_customer_30_day_average_spend
...
8. What is an incremental predicate, and how does it improve model performance?
   - An incremental predicate is an extra `WHERE` clause that dbt injects into the **destination side** of an incremental write — most visibly into the `DELETE` of a `delete+insert` strategy, or as a partition-pruning hint to a `MERGE`. Without a predicate the warehouse must consider the **entire historical target table** when looking for rows to replace, even when we know the matches must be in a narrow recent window. The predicate tells the planner "only this slice can possibly match," which lets it prune partitions / skip files / avoid full scans. In this project, both `fct_daily_revenue` and `fct_restaurant_daily` use `revenue_date >= current_date - interval '7 days'` so only the last week of the destination is touched.
